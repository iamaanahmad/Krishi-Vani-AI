...
Open the app and stay on the hero. Explain the intended farmer loop: an Odia voice question and leaf photo should lead to one understandable next step only when evidence is strong enough. The current demo proves that decision loop with two labelled fixture pairs, not real farmer media.

## 0:40–1:20 — honest demo boundaries

Point to **Offline demo** and **Only the labelled fixture pairs are interpreted**. Say that the demo media adapters are deterministic so the walkthrough works without credentials. Real speech recognition, real vision, and external dataset services are not connected.
...
1. the fixture transcript;
2. the evidence gate passing;
...
Show that the evidence gate stops, no citation is fabricated, and the answer asks for a clearer close-up plus KVK/extension review. This is the responsible-AI moment: the system knows when not to answer.
...
Close with the narrow plan: replace the same adapter interfaces with evaluated Odia ASR and rice vision, validate on consented Odisha field data with agronomists, then pilot through a KVK. The current prototype proves the safe product loop without overstating readiness.
